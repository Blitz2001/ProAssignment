...
export const validateNoContactInfo = (text) => {
    ...
    // Check for sharing-related phrases that might indicate contact info sharing
    const sharingPatterns = [
        /(share|sharing|give|send|text|call|contact|reach|message)\s+(me\s+)?(my\s+)?(email|phone|number|contact|whatsapp|telegram|instagram|facebook|twitter|snapchat|linkedin|discord|skype)/gi,
        /(my\s+)?(email|phone|number|contact|whatsapp|telegram|instagram|facebook|twitter|snapchat|linkedin|discord|skype)\s+(is|are|@|:)/gi,
        /(contact|reach|message|call|text|whatsapp|telegram)\s+(me\s+)?(at|on|via)/gi,
        /(add|follow|friend)\s+(me\s+)?(on|at)/gi,
    ];

    for (const patternStr of sharingPatterns) {
        const pattern = new RegExp(patternStr.source, patternStr.flags);
        if (pattern.test(text)) {
            return {
                isValid: false,
                errorMessage: "Sharing contact information is not allowed. Please communicate through this platform only."
            };
        }
    }
